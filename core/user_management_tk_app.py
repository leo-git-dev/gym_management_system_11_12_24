...
'''   def create_update_delete_user_menu(self):
        """
        Create the menu for updating or deleting users.
        """
        self.clear_root_widgets()
        ...
        ttk.Label(frame, text="Update/Delete User", font=("Helvetica", 14)).grid(row=0, column=0, columnspan=2, pady=10)

        ttk.Label(frame, text="Member ID:").grid(row=1, column=0, padx=5, pady=5, sticky="e")
        member_id_entry = ttk.Entry(frame)
        member_id_entry.grid(row=1, column=1, padx=5, pady=5, sticky="w")

        ttk.Label(frame, text="Field to Update:").grid(row=2, column=0, padx=5, pady=5, sticky="e")
        field_entry = ttk.Entry(frame)
        field_entry.grid(row=2, column=1, padx=5, pady=5, sticky="w")

        ttk.Label(frame, text="New Value:").grid(row=3, column=0, padx=5, pady=5, sticky="e")
        ...
        ttk.Button(
            frame,
            text="Delete User",
            command=lambda: self.delete_user(member_id_entry.get())
        ).grid(row=5, column=0, columnspan=2, pady=10)

        self.add_back_button(frame)

    def update_user(self, member_id, field, new_value):
        """
        Update a user's details in the database.
        """
        try:
            if not member_id or not field or not new_value:
                messagebox.showerror("Error", "Please provide Member ID, Field to Update, and New Value.")
                return

            MemberManagement.update_member(member_id, {field: new_value})
    ...
    def delete_user(self, member_id):
        ...
        try:
            ...
            if MemberManagement.delete_member(member_id):
    ...
    def add_back_button(self, frame):
        ...
        Add a 'Back' button to return to the main menu.
        """
        ttk.Button(frame, text="Back", command=self.create_main_menu).grid(row=10, column=0, columnspan=2, pady=10, sticky="ew")
...
class UserManagementApp:
    ...
    def create_main_menu(self):
        ...
        ttk.Label(frame, text="User Management System", font=("Helvetica", 16)).grid(row=0, column=0, columnspan=2, pady=20)

        ttk.Button(frame, text="Add User", command=self.create_add_user_menu).grid(row=1, column=0, columnspan=2, pady=10)
        ttk.Button(frame, text="View All Users", command=self.create_view_all_users_menu).grid(row=2, column=0, columnspan=2, pady=10)
        ttk.Button(frame, text="Search User", command=self.create_search_user_menu).grid(row=3, column=0, columnspan=2, pady=10)
        ttk.Button(frame, text="Update/Delete User", command=self.create_update_delete_user_menu).grid(row=4, column=0, columnspan=2, pady=10)
    ...
    def create_add_user_menu(self):
        ...
        ttk.Label(frame, text="Add User", font=("Helvetica", 14)).grid(row=0, column=0, columnspan=2, pady=10)

        user_type_dropdown = ttk.Combobox(
            frame, values=["Gym User", "Training Staff", "Wellbeing Staff", "Management Staff"]
        )
        user_type_dropdown.grid(row=1, column=0, columnspan=2, pady=10)
        user_type_dropdown.set("Select User Type")
    ...
    def get_gym_by_name(self, gym_name):
        """
        Retrieve a gym by its name.
        :param gym_name: The name of the gym.
        :return: A dictionary containing gym details or None if not found.
        """
        gyms = GymManager.view_all_gyms()
        return next((gym for gym in gyms if gym["gym_name"] == gym_name), None)
    ...
    def create_gym_user_menu(self):
        ...
        ttk.Label(frame, text="Gym:").grid(row=2, column=0, padx=5, pady=5)
        gym_dropdown = ttk.Combobox(frame, values=self.get_gym_names())
        gym_dropdown.grid(row=2, column=1, padx=5, pady=5)

        ttk.Label(frame, text="Membership Type:").grid(row=3, column=0, padx=5, pady=5)
        membership_dropdown = ttk.Combobox(
            frame, values=["Trial", "Standard", "Weekender", "Premium"]
        )
        membership_dropdown.grid(row=3, column=1, padx=5, pady=5)

        ttk.Label(frame, text="Join Date:").grid(row=4, column=0, padx=5, pady=5)
        join_date_calendar = Calendar(frame, selectmode="day", date_pattern="yyyy-mm-dd")
        join_date_calendar.grid(row=4, column=1, padx=5, pady=5)
    ...
    def create_training_staff_menu(self):
        ...
        ttk.Label(frame, text="Add Training Staff", font=("Helvetica", 14)).grid(row=0, column=0, columnspan=2, pady=10)
        ...
        ttk.Label(frame, text="Expertise:").grid(row=3, column=0, padx=5, pady=5)
        expertise_entry = ttk.Entry(frame)
        expertise_entry.grid(row=3, column=1, padx=5, pady=5)
        ...
        # Back Button
        back_button_frame = ttk.Frame(frame)
        back_button_frame.grid(row=9, column=0, columnspan=2, sticky="nsew", pady=10)
        self.add_back_button(back_button_frame)

        # Ensure row 9 expands to push content above
        frame.grid_rowconfigure(9, weight=1)
    ...
    def create_wellbeing_staff_menu(self):
        ...
        ttk.Label(frame, text="Activity:").grid(row=2, column=0, padx=5, pady=5)
        activity_dropdown = ttk.Combobox(frame, values=["Physiotherapy", "Nutrition"])
        activity_dropdown.grid(row=2, column=1, padx=5, pady=5)
    ...
    def create_management_staff_menu(self):
        ...
        ttk.Label(frame, text="Role:").grid(row=2, column=0, padx=5, pady=5)
        role_dropdown = ttk.Combobox(frame, values=["Reception", "Cleaning", "Cafeteria", "Security"])
    ...
    def add_schedule_fields(self, frame):
        ...
        Add fields for entering a schedule.
        """
        ttk.Label(frame, text="Day of Week:").grid(row=4, column=0, padx=5, pady=5)
        day_of_week_dropdown = ttk.Combobox(frame,
                                            values=["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday",
                                                    "Sunday"])
        day_of_week_dropdown.grid(row=4, column=1, padx=5, pady=5)
        self.day_of_week_dropdown = day_of_week_dropdown

        ttk.Label(frame, text="Start Time:").grid(row=5, column=0, padx=5, pady=5)
        start_time_entry = ttk.Entry(frame)
        start_time_entry.grid(row=5, column=1, padx=5, pady=5)
        self.start_time_entry = start_time_entry

        ttk.Label(frame, text="End Time:").grid(row=6, column=0, padx=5, pady=5)
        end_time_entry = ttk.Entry(frame)
        end_time_entry.grid(row=6, column=1, padx=5, pady=5)
        self.end_time_entry = end_time_entry
    ...
    def add_back_button(self, frame):
        ...
        Add a 'Back' button to return to the main menu.
        """
        ttk.Button(frame, text="Back", command=self.create_main_menu).grid(row=7, column=0, columnspan=2, pady=10)
    ...
    def create_search_user_menu(self):
        ...
        # Create a frame for the search menu
        frame = ttk.Frame(self.root)
        frame.grid(sticky="nsew", padx=10, pady=10)

        # Title label
        ttk.Label(frame, text="Search User", font=("Helvetica", 16)).grid(row=0, column=0, columnspan=3, pady=10)

        # Search by Member ID
        ttk.Label(frame, text="Member ID:").grid(row=1, column=0, padx=5, pady=5)
        search_id_entry = ttk.Entry(frame)
        search_id_entry.grid(row=1, column=1, padx=5, pady=5)

        # Search by Name
        ttk.Label(frame, text="Name:").grid(row=2, column=0, padx=5, pady=5)
        search_name_entry = ttk.Entry(frame)
        search_name_entry.grid(row=2, column=1, padx=5, pady=5)

        # Results text box
        search_results = tk.Text(frame, height=15, wrap="word")
        search_results.grid(row=3, column=0, columnspan=2, padx=5, pady=10, sticky="nsew")
        ...
        ttk.Button(
            ...
            text="Search",
            command=lambda: self.search_user(
                search_id_entry.get(), search_name_entry.get(), search_results
            )
        ).grid(row=4, column=0, columnspan=2, pady=10)
    ...
    def search_user(self, member_id, name, result_box):
        """
        Search and display user details in the result box.
        """
        try:
            result_box.delete("1.0", tk.END)  # Clear previous results
            user = MemberManagement.search_member(member_id=member_id, name=name)
            if user:
                result_box.insert(tk.END, f"{user}\n")
            else:
                result_box.insert(tk.END, "No matching user found.")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to search user: {e}")
    ...
    def create_update_delete_user_menu(self):
        ...
        # Title label
        ttk.Label(frame, text="Update/Delete User", font=("Helvetica", 16)).grid(row=0, column=0, columnspan=3, pady=10)
        ...
        ttk.Label(frame, text="Field to Update:").grid(row=2, column=0, padx=5, pady=5)
        field_entry = ttk.Entry(frame)
        field_entry.grid(row=2, column=1, padx=5, pady=5)

        ttk.Label(frame, text="New Value:").grid(row=3, column=0, padx=5, pady=5)
        value_entry = ttk.Entry(frame)
        ...
        ttk.Button(
            ...
            text="Update User",
            command=lambda: self.update_user(
                member_id_entry.get(), field_entry.get(), value_entry.get()
            )
        ).grid(row=4, column=0, columnspan=2, pady=10)
